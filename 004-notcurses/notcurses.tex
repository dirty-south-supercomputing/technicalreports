...
\textbf{The code written for this book attempts to minimize use of vertical space
...
the first step of \texttt{notcurses\_render()}.
...
thus critical to understand that \textbf{\texttt{notcurses\_rende()} can block
...
To warm up and get limber, let's create 7 \texttt{ncplane}s, one for each type
...
\caption{Drawing a single tetrimino (from \texttt{tetrimino.c}).}
...
\caption{Displaying the tetriminos (from \texttt{tetrimino.c}).}
\end{listing}

Our \texttt{main()} sets the locale, initializes the terminal, and draws the
seven pieces. Each piece has a one-letter name; for now, we draw with those
glyphs. The pieces are monochromatic. This renders familiar shapes in the correct colors
(Figure~\ref{fig:tetriminos-1}), but other than that, it's (like most
classic Curses programs) pretty ugly. I wouldn't want to play with these
pieces.
...
display, we need the bold, and on a display which can't do bold, maybe we
  get a color difference. Also, we want bold even if we have color, because
  until we see the base color, there's no reason to think of the initial selection
  as ``bright''.}. The space bar will advance among the pieces in one direction,
...
\caption{Handling input and highlighting a selection (from \texttt{tetrimino.c}).}
