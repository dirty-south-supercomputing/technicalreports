...
{
  name={ANSI X3.4-1968},
  description={A 7-bit character encoding also known as ASCII. Its code set is a proper set of UCS, and its encoding is a proper subset of \gls{utf8} (but \textit{not} \gls{utf16}, nor \gls{utf32})}
}
...
{
  name={ECMA-6},
  description={See \Gls{isoiec646}.}
}

\newglossaryentry{ecma35}
{
  name={ECMA-35},
  description={See \Gls{isoiec2022}.}
}
...
{
  name={ECMA-94},
  description={\Gls{isoiec8859} parts 1, 2, 3, and 4.}
}
...
{
  name={HSV},
  description={Hue/Saturation/Value FIXME}
}
...
\newglossaryentry{isoiec646}
{
  ...
  description={``Information technology — ISO 7-bit coded character set for
    ...
    edition, the IRV is explicitly identical to ASCII. Twinned with
    \Gls{itut50}. Ratified as \Gls{ecma6}.}
}
...
\newglossaryentry{isoiec2022}
{
  ...
  description={``Information technology—Character code structure and extension techniques''.
   An absolute mess of a standard.
...
description={A fixed-length encoding of the Basic Multilingual Plane. Each
  ...
  used. An amendment to \Gls{isoiec10646} extended UCS-2 to UTF-16}
}
...
geometry changes (see Chapter~\ref{section:uiwidgets}, \textit{UI Widgets}).
...
\item{\texttt{bool suppress\_banner}: At startup, Notcurses emits some
    ...
    field to disable these outputs.}
