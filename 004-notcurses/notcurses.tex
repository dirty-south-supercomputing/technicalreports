...
\begin{center}
  ...
  \vspace{.25in}
    For David Foster Wallace, who urged us with nothing but smiles on to great and greater demonstrations of effort, so that after we'd got our third and final wind and had to vomit, there was little inside to vomit and the spasms passed quickly and an east breeze blew cooler past us and we felt clean and could breathe\ldots\\
    and who taught us that talent is sort of a dark gift, that talent is its own expectation: it is there from the start and either lived up to or lost. Who taught us How to Win, Later.}\\

  \vspace{1in}\ldots but mostly for Emily, a better version of me.
\end{center}
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item{their lower effective resolution typically implies bandwidth requirements.}
\end{denseitemize}
...
but by evil rulers and spiritual powers. In too often goes unmentioned that the
...
are typically rectilinear applications, as opposed to line-oriented CLIs.

Given the same interactive task\footnote{None of these relations are
  fundamental, but arise from the reality of GUI toolkits. You can write ass
  code in any interface.},

\begin{denseitemize}
\item{A TUI implementation is almost certainly a smaller memory and disk footprint than a GUI,}
\item{a good TUI implementation might introduce less latency,}
...
For over two decades, NCURSES (a free software implementation of the X/Open Curses
specification, plus extensions\cite{ncursesfaq}) has been a ubiquitous go-to for implementing
TUIs.

\begin{wrapfigure}{r}{0.5\linewidth}
  \includegraphics[]{media/emacs-xerox.jpg}
  \caption{Put not your trust in hackers singing idle hagiographies of Xerox PARC.}
\end{wrapfigure} 

\textbf{FIXME define character graphics, terminal, that good stuff}
