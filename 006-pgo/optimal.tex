...
{$\mathbfit{C\kern -1ptP}$\,1500 bound sorted by $\frac{\mathbfit{BS}}{\mathbf3}$ (Great League)\label{subsec:cp1500a}}
...
For CP1500 (\autoref{subsec:cp1500a}, \autoref{subsec:cp1500g}), when a form can max out, i.e.\ is not restricted by the CP bound,
  the maximum advantage tends to be 10\%--25\%, with a few outliers around 50\%.
It otherwise quickly drops to 4\% or less.
In CP2500 (\autoref{subsec:cp2500a}, \autoref{subsec:cp2500g}), the advantage available from IVs is even smaller, as we would expect.
...
{CP2500 bound sorted by $\frac{\mathbfit{BS}}{\mathbf3}$ (Ultra League)\label{subsec:cp2500a}}
See \autoref{subsec:cp1500a} for important notes.
\input{out/cp2500a}
\subsection[CP1500 bound sorted by \protect{$\sqrt[3]{\BP\,}$} (Great League)]%
{CP1500 bound sorted by $\sqrt[3]{\mathbfit{BP}\;}$ (Great League)\label{subsec:cp1500g}}
See \autoref{subsec:cp1500a} for important notes.
...
{CP2500 bound sorted by $\sqrt[3]{\mathbfit{BP}\;}$ (Ultra League)\label{subsec:cp2500g}}
See \autoref{subsec:cp1500g} for important notes.
This table uses the geometric mean $\sqrt[3]{\BP{}\,}$ instead of the arithmetic mean.
Since Shadow forms don't change under the geometric mean, they have been elided.
\input{out/cp2500g}
...
Configurations of forms are selected to maximize $\sqrt[3]{\BP}$, and these
 configurations are sorted by bulk (geometric mean of \MHP{} and \Eff{D}).
\input{out/bulk1500}
