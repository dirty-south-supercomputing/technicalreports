...
We multiply each of the $\mathit{Mod}$ values by the Pokémon's \CPM{} to generate
  its \Eff{A}, \Eff{D}, and \Eff{S}.
$M_\mathrm{Shadow}$ is $\frac{6}{5}$ if the Pokémon is a Shadow Pokémon,
  and 1 otherwise.
We multiply \Eff{A} by this Shadow bonus, and divide \Eff{D} by it.
This manifests a 20\% bonus to \Eff{A} and a 16.7\% penalty to \Eff{D}.
$M_\mathrm{ASE}$ and $M_\mathrm{DSE}$ are the Pokémon's attack and defense multipliers due any active stat effect (\autoref{sec:buffs}).
Recall that the stat effect levels take the values -4--4, inclusive.
They are multiplied into \Eff{A} and \Eff{D} in 3x3 battle contexts (\autoref{sec:3x3})
...
The system behind the scheme of \autoref{table:buffs} is that the negative levels $-n$
 ...
 $\frac{4 + n}{4}$.
\begin{align*}
 \Eff{A} &= \Mod{A} \cdot \CPM \cdot M_\mathrm{Shadow} \\
 \Eff{D} &= \frac{\Mod{D} \cdot \CPM}{M_\mathrm{Shadow}}
\end{align*}
\Eff{S} determines the Pokémon's Maximum Hit Points (\MHP).
...
\begin{align*}
 \Eff{S} &= \mathit{Mod_S} \cdot \CPM \\
 \MHP &= \min{1, \lfloor \Eff{S} \rfloor}
\end{align*}
