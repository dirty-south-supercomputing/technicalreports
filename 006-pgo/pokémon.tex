...
\begin{align*}
  ...
  \MHP{} &= \min{10, \lfloor \Eff{S} \rfloor}
\end{align*}
...
\begin{align*}
  P &= \Eff{A} \cdot \Eff{D}\\
  &= \frac{6}{5}\Mod{A} \cdot \frac{5}{6}\Mod{D} \cdot \CPM{}^2
\end{align*}
Using a sum function such as the arithmetic mean will usually show a difference.
When $\Mod{A} < \frac{5}{6}\Mod{D}$, the average goes down.
When $\Mod{A} = \frac{5}{6}\Mod{D}$, the average is unchanged,
 and the average increases if $\Mod{A} > \frac{5}{6}\Mod{D}$.
...
item & Evolution requires an item (\autoref{table:itemevolutions})\\
legendary & Legendary \\
lucky & Lucky Pokémon \\
male & Males\\
...
shadow & Unpurified Shadow Pokémon (\autoref{sec:shadow})\\
shiny & Shiny Pokémon\\
special & Exclusive move(s) (\autoref{sec:tms})\\
tradeevolve & Evolution discount if traded (\autoref{table:tradeevolution})\\
