...
Prefix a species with '+' to search for its genus.
...
\texttt{cpNUM} and \texttt{hpNUM} match exactly \texttt{NUM} CP and HP;
\texttt{cpNUM-NUM} and \texttt{hpNUM-NUM} provide range support, and
\texttt{cp-NUM} and \texttt{hp-NUM} inclusive ceilings.
...
\end{tabular}
\caption{Search terms for Pokémon storage/Pokédex}
