...
We multiply each of the $Mod$ values by the Pokémon's CPM to generate
 its $\mathit{Eff_A}$, $\mathit{Eff_D}$, and $\mathit{Eff_S}$.
$M_{Shadow}$ is $\frac{6}{5}$ if the Pokémon is a Shadow Pokémon,
  and 1 otherwise.
We multiply $\mathit{Eff_A}$ by this Shadow bonus, and divide $\mathit{Eff_D}$ by it.
This manifests a 20\% bonus to $\mathit{Eff_A}$ and a 16.7\% penalty to $\mathit{Eff_D}$.
...
They are multiplied into $\mathit{Eff_A}$ and $\mathit{Eff_D}$ in 3x3 battle contexts (\autoref{sec:3x3})
...
The system behind the scheme of \autoref{table:buffs} is that the negative levels $-n$
 ...
 $\frac{4 + n}{4}$.
\begin{align*}
 \mathit{Eff_A} &= Mod_A \cdot CPM \cdot M_{Shadow} \\
 \mathit{Eff_D} &= \frac{Mod_D \cdot CPM}{M_{Shadow}}
\end{align*}
$\mathit{Eff_S}$ determines the Pokémon's Maximum Hit Points (MHP).
...
\begin{align*}
 \mathit{Eff_S} &= Mod_S \cdot CPM \\
 MHP &= \min{1, \lfloor \mathit{Eff_S} \rfloor}
\end{align*}
