...
Size and height are the criteria for winning Showcases, otherwise have no functional effects.
...
Such Pokémon have their own Pokédex (\autoref{sec:dexen})\footnote{A Pokémon with an IV of 0/0/0
 is colloquially known as a ``nundo'' or ``0\%er''. They have no unique Pokédex, because they are worthless pieces of shit,
  and don't let anyone tell you otherwise. Consult \autoref{chap:speciesbytype} to see that 0/0/0 is never an optimal IV.}.
While 15/15/15 is generally the optimal IV, this is not always true in \CP-bounded
  ...
  and \autoref{chap:speciesbytype} for tables of optimal IVs under different \CP{} bounds.
\begin{table}
\centering
\footnotesize
\begin{tabular}{ll}
...
Finally, Silver and Gold Bottle Caps enroll a Pokémon of at least Good Buddy status in ``hypertraining''.
...
The tasks are assigned serially, and Hypertraining must be completed within a year's time.
Once accomplished, the Pokémon acquires the new IV.
...
\begin{tabular}{rp{.25\textwidth}p{.25\textwidth}p{.30\textwidth}}
IV & \IV{A} wins & \IV{D} & \IV{S}\\
  1 & Gym battle & Buddy snapshot & Spin Photodisc\\
  2 & GO Rocket Grunt & Play with Buddy & Earn a Candy w/ Buddy\\
  3 & Raid & Power up a Pokémon of the same type as trainee & Research task\\
  4 & 2 Gym battles & Catch 15 species & Hatch an egg\\
  5 & 2 GO Rocket Grunts & Catch 30 Pokémon & Explore 1 km\\
  6 & 5 Raids & 7 days with a Buddy snapshot & Spin 40 Photodiscs\\
  7 & 20 Gym Battles & 7 days with Buddy play & Earn 3 Candy w/ Buddy\\
  8 & 10 Raids & Power up Pokémon of the same type as trainee 15 times & 30 Researches\\
  9 & 15 GO Rocket members & Catch 45 species & Hatch 15 eggs\\
 10 & 2 GO Rocket Leaders & Catch 150 Pokémon & Earn 4 Candy w/ Buddy\\
 11 & 20 Raids & 15 days with Buddy snapshot & Spin 75 Photodiscs\\
 12 & 40 Gym battles & 15 days with Buddy play & Earn 5 Candy w/ Buddy\\
 13 & 40 GO Rocket members & Power up Pokémon of the same type as trainee 30 times & 45 Researches\\
 14 & 5 GO Rocket Leaders & Catch 60 species & Hatch 30 eggs\\
 15 & 40 Raids & Catch 1500 Pokémon & Earn 10 Candy w/ Buddy\\
\end{tabular}}
\caption{Hypertraining tasks\label{table:hypertraining}}
\end{table}
...
The probability of a random Pokémon being a hundo is actually
  ...
  (e.g.\ the chance of a 15/15/15 for a lucky trade is 1 in 64, whereas
...
& 0 & 1--min(TL, 30) \\
\rowcolor{Gray!25}
\multirow{-2}{*}{Wild catch} & 4 w/ weather boost & 1--min(TL, 30) + 5 \\
Hatch & 10 & min(TL, 20) \\
\rowcolor{Gray!25}
Shadow Raid & 6 & 20 (25 w/ boost) \\
  Raid & 10 & 20 (25 w/ boost) \\
\rowcolor{Gray!25}
Super Mega Raid & 11 & 20 (25 w/ boost)\\
Max Battle & 10 & 20 \\
\rowcolor{Gray!25}
Research & 10 & 15 \\
Team GO Rocket & 0 & 8 \\
...
& 12 (lucky trade) & \\
  & & 35--40 \\
  \multirow{-2}{*}{Mighty Pokémon} & \multirow{-2}{*}{13} & 40--45 w/ boost\\
\end{tabular}
...
This discount stacks with the 10\% purification discount, so Stardust costs for
...
Range suffixes take the form \texttt{NUM} for an exact match of \texttt{NUM},
  ...
  \texttt{NUM-} for an inclusive floor,
...
\footnotesize
\raggedright
\textit{Not tagged:} \texttt{!\#}

\textit{Two charged attacks:} \texttt{!@3move}

\textit{Can be evolved into new dex entries:} \texttt{evolvenew\&evolve}

\textit{Have at least two instances of the species:} \texttt{count2-}

\textit{Can take place in Max Battles:} \texttt{maxmove1-}

\textit{Can become a hundo via a silver bottle cap:}

\texttt{4attack,4defense\&4attack,4hp\&4defense,4hp\&!4*\&!shadow}
\end{tipbox}
...
I am limited to 850 Pokémon, yet have never found myself wishing I had thousands to draw from.
\begin{table}
\centering\small
\begin{tabular}{ll}
Keyword & Matches \\
