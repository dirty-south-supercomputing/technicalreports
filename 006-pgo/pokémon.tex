...
$M_\mathrm{Shadow}$ is $\frac{6}{5}$ if the Pokémon is a Shadow Pokémon, and 1 otherwise.
Multiplying \Eff{ARaw} by $M_\mathrm{Shadow}$ yields \Eff{A}.
Multiplying \Mod{D} by \CPM{} and dividing by $M_\mathrm{Shadow}$ gives us \Eff{D}.
This manifests a 20\% bonus to \Eff{A} and a 16.7\% penalty to \Eff{D}.
\begin{align*}
  \Eff{ARaw} &= \Mod{A} \cdot \CPM\\
  \Eff{A} &= \Eff{ARaw} \cdot M_\mathrm{Shadow} \\
