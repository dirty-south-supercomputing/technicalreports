...
\rowcolor{Gray!25}
  ...
  Win 30 raids\\
  74 & 30 & Level up (\textit{not} unlock) 20 Max Moves (\autoref{sec:dmaxgmax}),\newline
  Explore 200 km,\newline
...
\input{out/xpupgrades}
Level 80 awards a Mewtwo encounter.
Levels 7, 8, 15, 18, and 20 introduce Special Research (\autoref{sec:research}).
% FIXME i don't think these exist?
%, as do 73, 75, 78, and 80 (these under the guise of ``challenges'').
\begin{table}
\centering\footnotesize
\begin{tabular}{rp{0.75\textwidth}}
Level & Features/perks unlocked \\
\Midrule
2 & Eggs (\autoref{sec:eggs})\\
5 & Gyms, Raids (\autoref{sec:raids})\\
7 & Routes, ``An Intriguing Incense''\\
8 & Team GO Rocket, ``A Troubling Situation'' \\
10 & Trades, GBL/Trainer Battles\\
13 & Weekly Challenges\\
14 & Pokémon storage tagging\\
15 & Daily Adventure Eggs, ``A Mythical Discovery'' \\
18 & Mega Evolution (\autoref{sec:mega}), ``A Mega Discovery''\\
20 & Max Battles (\autoref{sec:maxbattles}), ``To The Max!''\\
...
Additional storage can be added in increments of 50, up to a total of 11,650.
Each upgrade costs 200 Pokécoins.
Level upgrades (\autoref{table:levelupgrades}) add another 350, for a maximum of 12,000.
...
The Bag can be upgraded to store up to 11,000 items.
Here again, each upgrade costs 200 Pokécoins, and increases capacity by 50 items.
Storage awards due to level upgrades (\autoref{table:levelupgrades}) add another 750, for a maximum of 11,750.
...
The Trainer can typically carry up to ten Purified Gems.
...
The Trainer can collect no more than 50,000 units of any Mega Energy,
...
As I write this about fourteen months later, I have caught 126,043 Pokémon, and retained 1,025---a little more than 0.8\%.
...
\Midrule
  ...
  Spin Photo Disc & 50\\
  Send a Gift & 200\\
  Spin new Photo Disc & 250\\
  Spin first Photo Disc of the day & 500\\
  Evolution, new main Pokédex registration & 1000\\
  Spin first Photo Disc of the 7th day of streak & 2500\\
...
Stardust can also be collected as rewards in the GO Battle League,
...
\Midrule
Audino & 2100 \\ %& 2625\\
\rowcolor{Gray!25}Cloyster & 1200 \\%& 1500\\
Shellder, Chimecho & 1000 \\%& 1250\\
\rowcolor{Gray!25}Alolan Persian, Starmie, Vespiquen, Garbodor & 950 \\%& 1188\\
Alolan Meowth, Staryu, Sableye, Combee, Trubbish & 750 \\
\rowcolor{Gray!25}Parasect, Persian, Breloom, Amoonguss, Shiinotic & 700\\% & 875\\
Flamigo & 600 \\
\rowcolor{Gray!25}Paras, Meowth, Delibird, Shroomish, Foongus,\newline
Morelull, and stage 3 Pokémon not yet specified & 500\\% & 625\\
Stage 2 Pokémon not yet specified & 300 \\%& 375\\
\rowcolor{Gray!25}All other Pokémon & 100 \\%& 125\\
\end{tabular}
\caption{Base Stardust awards for catching Pokémon\label{table:stardust}}
\end{table}
...
Its loss of existence is your gain of one tasty Candy (and possibly a Candy XL).
...
Very rarely, spinning Photo Discs will result in Beedrill Mega energy.
...
Very few of my Pokémon have the optimal IVs for their Leagues (\autoref{chap:bounded}),
  ...
  yet my rating rarely dips below 2100.
...
\begin{itemize}
  ...
  \item \textbf{Today:} Field research---short-term (usually) tasks acquired by spinning Photo Discs.
    Despite the name of the tab, field research persists until completed.
    Up to four field research tasks can be active at a time.
    A Pokéstop will not assign more than one field research per day, and Gyms do not assign research.
    Field research tasks can be dismissed without completion.
    This tab also shows any Pokémon in Gyms and Power Stops.
  \item \textbf{Special:} Longer-term research (\autoref{table:specresearch}) comprising the game's plot, so far as one exists.
...
Field researches include:
\begin{itemize}
  \item \textbf{Catch \textup{N} Pokémon}. The Pokémon can be the same species.
    Catches after raids, Max Battles, and research count.
    Hatches, evolutions, form changes, and trades do not count.
  \item \textbf{Power up \textup{N} Pokémon}. Powering up the same Pokémon \textit{M} times counts for \textit{M}.
     Purifying a Shadow Pokémon (\autoref{sec:shadow}) does not count, even if it advances levels.
     Evolution and form change do not count. Adding a second move does not count. Upgrading a Max Move
     does not count.
   \item \textbf{Evolve \textup{N} Pokémon}.
      Evolving the same Pokémon twice counts as two evolutions.
      Form change does not count.
    \item \textbf{Catch \textup{N typelist} Pokémon}. See \textit{Catch \textup{N} Pokémon} above.
      A dualtype (\autoref{sec:dualtypes}) Pokémon needs only have one type from the typelist
       to count. A dualtype Pokémon with both types on the typelist only counts once.
     \item \textbf{Power up \textup{N typelist} Pokémon}. See \textit{Power up \textup{N} Pokémon}
       and \textit{Catch \textup{N typelist} Pokémon} above.
     \item \textbf{Evolve \textup{N typelist} Pokémon}. See \textit{Evolve \textup{N} Pokémon}
       and \textit{Catch \textup{N typelist} Pokémon} above.
  \item \textbf{Catch \textup{N} different species of Pokémon}.
    Different species of the same family count.
    Different forms do not count.
  \item \textbf{Win \textup{N} raids}. The raids can be the same species.
      Remote raids count.
      Max Battles do not count.
      Successfully catching the Pokémon is not necessary.
  \item \textbf{Make \textup{N quality} throws}.
      Throws of a higher quality than that specified count.
      Throwing \textit{M} sufficient quality times in a single encounter counts for \textit{M}.
      Successfully catching the Pokémon is not necessary.
  \item \textbf{Make \textup{N quality} throws in a row}.
      Any throw which is not of at least the specified count breaks an active streak.
  \item \textbf{Use \textup{N} Berries to help catch Pokémon}.
      All Berry types count.
      Using \textit{M} Berries can count for \textit{M}.
      Use is credited when the Pokémon is caught, runs away, or breaks out of the ball.
      Use is credited when the Trainer leaves the encounter, but if the encounter
       is restarted, this Berry will not be credited again.
  \item \textbf{Hatch \textup{N} Eggs}.
    \textit{M} simultaneous hatches count for \textit{M}.
  \item \textbf{Explore \textup{N} km}.
    Any movement tracked by Adventure Sync counts.  
  \item \textbf{Spin \textup{N} Pokéstops or Gyms}.
    Spinning the same Photo Disc \textit{M} times counts for \textit{M}.
  \item \textbf{Take \textup{N} snapshots of wild Pokémon}.
    Pokémon in storage do not count.
    % FIXME more...
\end{itemize}
...
The vast majority are functionally useless, though several levels beyond 70 require earning
...
\begin{tabular}{p{.3\textwidth}p{.4\textwidth}rrrr}
Name & Counts & B & S & G & P\\
\Midrule%\endhead
Pilot & km of Trades & 1k & 100k & 1M & 10M\\
Backpacker & Photo Disc spins & 100 & 1k & 2k & 50k\\
...
\end{tabular}\caption{Unstructured medals\label{table:medals}}\end{table}
%\end{longtable}
%\endgroup
%\include{items}
