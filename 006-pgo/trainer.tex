...
Should they capture a Pokémon of Level 32.5, or receive one via trade, its
...
The Trainer has a Bag capable of storing 350 items (strangely, not all items
  ...
  which you presumably shove down the front of your Trainer uniform).
...
It seems plausible that the limit is one less than a large power of 2:
  4,294,967,295 seems as good a guess as any\footnote{If the limit is driven by UI requirements, it's likely lower.}.
...
\section{Pokédexen}
\label{sec:dexen}
The Trainer has 10 Pokédexen\footnote{``Pokédexes'' to the uncultured.}:
  ``Pokémon'', ``Shiny'', ``XXL'', ``XXS'', ``G-Max'', ``Mega'', ``Shadow'',
  ``Purified'', ``🟉100\%'', and ``Lucky''.
All 10 start empty, and several start locked.
\textbf{FIXME details}
...
Shiny Shadow Pokémon and Shiny Purified Pokémon are registered to the
  ``Shadow'' and ``Purified'' Pokédexen respectively, not\textit{not} ``Shiny''.
