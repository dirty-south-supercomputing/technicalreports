...
\section{$M_C$, the typing multiplier}
We must first determine the common multiplier due Typing:
...
Note that the change due to a $T$ of 1 (60\%) is very close to the negative change
 when $T$ is -2 (-60.94\%). A $T$ of -1 is a smaller (-37.5\%) loss. This does not
 mean that an Attack ineffective against one of the defender's types and
