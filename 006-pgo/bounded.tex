...
Common functions include CP itself, the arithmetic mean ($\frac{Mod_A + Mod_D + MHP}{3}$),
  and the geometric mean ($\sqrt[3]{Mod_A \ctimes Mod_D \ctimes MHP}$)\footnote{I personally believe the geometric mean to best match Pokémon GO's
  mechanics, but there is no true ``best'' fitness function---optimal IVs are
  strongly dependent upon the base stats, and weakly dependent upon the
  opponent.}.
