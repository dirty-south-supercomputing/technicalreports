...
Since the observable \MHP{} increases only stepwise with respect to \Eff{S},
  there is a stepwise overvaluation of \Mod{S}: a \ivlev{0}{14}{13}{30.5} Clodsire has the same \MHP{}
  as a \ivlev{0}{14}{14}{30.5} at the same level (213), but a \CP{} of 1499 as
...
Since \CPM{} is also used to calculate \MHP{} from \Mod{S}, one might
...
Furthermore, when two Pokémon launch charged attacks on the same turn in a Trainer
  Battle, \Eff{ARaw} determines which goes first (\autoref{sec:3x3}).
...
All else being equal, it's usually more important to reduce your Pokémons'
  weaknesses than to increase their resistances, since your opponent is not selecting a random attack type.
...
At first glance, \Eff{A} scales arbitrarily, while \Eff{D} yields diminishing returns.
...
Damage greater than the opponent's hit points is wasted, and thus can't meaningfully scale
 beyond the 429 hit points of a \ivlev{15}{15}{15}{50} Blissey.
Meanwhile, an \Eff{D} even four times \Eff{A} is still a factor an order of magnitude
 less than the power of a charged attack.
With that said, each increase in \Mod{A} is worth a constant increase in
 damage (leaving aside the floor function), whereas each increase in \Mod{D}
...
For the purposes of \Eff{A}, \Eff{D}, \MHP, and CMP, a \ivlev{0}{0}{9}{N} Beedrill
 and a \ivlev{8}{6}{0}{N} Ariados are the exact same thing, and will have equal CP.
...
In unbounded raids, Max Battles, Team GO Rocket battles, and the Master League,
  \ivlev{15}{15}{15}{N} is always optimal, though it is possible for \ivlev{15}{15}{M}{N} to tie.
As an example, a Sigilyph (base STA 176) at level 22 has an \MHP{} of 119 with
  either a 15 or a 14 \IV{S}: there is literally no difference between the two values.
...
At level 50, an \IV{A} of 15 boosts \Mod{A} by 12.6045.
...
My (soft) rule is to only level up so long as the \CPM{} fraction exceeds the IV fraction:
 \ivlev{14}{14}{14} is 93.3\% of \ivlev{15}{15}{15}, so I don't level that Pokémon
...
You'll use fast attacks more than anything else, so choose wisely.
...
\subsection{Seeding and building\label{subsec:seeding}}
