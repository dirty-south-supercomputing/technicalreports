...
Despite this, a Trainer can have up to 650 other Trainers as ``friends''.
How can this capacity be exploited?

When two Trainers agree to friendship, they are assigned a friendship level of zero.
The friendship can progress through five levels, based on the number of calendar
  days the Trainers interact (\autoref{table:friendlevels}).
Reaching a new level awards XP to both Trainers, and the awards for levels 4 and 5 are substantial.
...
Completing a Weekly Challenge together awards seven days' interactions at once.
Addition of and building friendship with other Trainers, facilitated by online friend code parlors,
  is the surest way to progress through levels.
...
\begin{tabular}{lrrrrr}
  & Good & Great & Ultra & Best & Forever \\
\Midrule
  Days & 1 & 7 & 30 & 90 & 180\\
  kXP  & 3 & 10 & 50 & 100 & 150 \\
  XP/friend-day & 3,000 & 1,857 & 2,100 & 1,811 & 1,738 \\
  Damage bonus &  & 5\% & 7\% & 10\% & 12\% \\
  Ball bonus &  & 1 & 2 & 4 & 4 \\
  Trade discount &  & 20\% & 92\% & 96\% & 96\% \\
\end{tabular}
...
Keyword & Matches \\
\Midrule
\texttt{giftable} & Friends to whom you can currently send a gift (\autoref{sec:gifts})\\
\texttt{friendlevelN} & Friend level with standard syntax (\autoref{sec:searching})\\
\texttt{interactable} & Friends with whom you've not today interacted \\
\texttt{lucky} & Lucky friends (\autoref{sec:trades})\\
  \texttt{remotetrade} & Friends with whom you can make a Remote Trade (\autoref{sec:trades})\\
\end{tabular}
\caption{Search terms for friends/Pokédex\label{table:searchtermsfriends}}
\end{table}
...
\begin{tipbox}[title=What's that Pikachu?]
Sometimes the gift icon will include a Pikachu head.
This (somewhat opaquely) indicates the presence of a sticker.
As noted earlier, stickers are useless, and thus this indicator is useless.
\end{tipbox}
...
Trainers who share both friendship and spatial proximity (or just Forever Friends status) can trade Pokémon.
...
The traded Pokémon's level (\autoref{sec:plevel}) is truncated if it is not an integer (a level 20.5
 will reduced to level 20).
Level is otherwise unchanged, as are shininess, costuming, background, size, battles won, nickname, and attacks.
...
Certain Pokémon can be evolved at no cost once traded (\autoref{table:tradeevolution}),
 and this can be a source of strong Pokémon in the early game.
\begin{table}
\centering
\begin{tabular}{lr}
Type & Undiscounted cost \\
\Midrule
Registered & 100 \\
Unregistered, reg.\ Special & 20,000 \\
Unregistered Special & 1,000,000 \\
\end{tabular}
...
% https://pokemongo.com/en/news/new-years-2026
Each Trainer has a number of ``guaranteed Lucky trades'' (currently 45); when a Pokémon
 captured prior to 2021 is traded, and the receiving Trainer has guaranteed Lucky trades
 remaining, the trade will be Lucky.
...
Routes have their own additional spawn points, and can thus lead to more encounters, and even route-specific spawns.
Routes are the only way to gather Zygarde Cells (\autoref{sec:zygarde}).
