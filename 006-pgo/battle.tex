...
\[ M_\mathrm{B} = 0.65 \cdot M_\mathrm{G} \cdot M_\mathrm{E} \]
Recall that the stat effect levels take the values −4--4, inclusive.
Their effects are determined via \autoref{table:buffs}.
$M_\mathrm{E}$ is the ratio of the attacking Pokémon's offensive buff effect to that of the opponent's defensive one (\autoref{sec:buffs}).
...
\centering
\begin{tabular}{l r}
Summary & $M_\mathrm{G}$ range (no Shield) \\
\Midrule
``Excellent!'' & 1 \\
``Great!'' & [0.75, 1) \\
``Nice!'' & [0.5, 0.75) \\
...
\[ D = \left\lfloor P \cdot \frac{\Eff{A}}{\Eff{D}} \cdot M_\mathrm{C} \cdot M_\mathrm{B} \right\rfloor + 1 \]
...
The Pokémon that goes first (``Charged Move Priority'') is the one with the
  higher \Eff{ARaw} (recall that this is base attack plus $\mathit{IV}_\mathrm{A}$, times \CPM, but does not reflect the Shadow bonus).
