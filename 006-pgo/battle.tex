...
\[ D_A = P \times \mathit{Eff_A} \times M_{STAB} \times M_B \]
...
\[ D_D = \frac{D_A}{\mathit{Eff_D}} \times 1.6^T \]

We can recover our defined multipliers:

\[ D_D = P \times \frac{\mathit{Eff_A}}{\mathit{Eff_D}} \times M_C \times M_B \]
...
\[ D = \min{1, \left\lfloor P \times \frac{\mathit{Eff_A}}{\mathit{Eff_D}} \times M_C \times M_B \right\rfloor } \]
...
\[ F = \mathit{Eff_A} \times P \times M_{STAB} \times \mathit{Eff_D} \times MHP \]
...
\[ F = \mathit{Eff_A} \times \frac{P_{Fast}}{T_{Fast}} \times M_{STAB-Fast} \times \mathit{Eff_D} \times \mathit{Eff_S} \]
...
The Pokémon that goes first (``Charged Move Priority'') is the one with the
  higher $\mathit{Eff_A}$ (recall that this is base attack plus $IV_A$, times CPM).
