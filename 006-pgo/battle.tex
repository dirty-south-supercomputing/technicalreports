...
$T$ is the number calculated in \autoref{chap:types} using the attack type
 and the defender's type, and takes an integer value from $[−3, 2]$.
It might be useful to recognize 1.6 as $\frac{8}{5}$ and 1.2 as $\frac{6}{5}$.
...
We see that $M_\mathrm{C}$ ranges
...
In Raids, Max battles, and Gym takeovers, a number of effects come into play:
\[ M_\mathrm{B} = 0.5 \cdot M_\mathrm{D} \cdot M_\mathrm{F} \cdot M_\mathrm{W} \cdot M_\mathrm{M} \]
\begin{itemize}
 \item $M_\mathrm{D}$ is 0.25 if the attack is dodged, and 1 otherwise.
...
\centering
 ...
 \item $M_\mathrm{M}$ from Mega Pokémon \textit{actively} involved (i.e. they
   must be fighting, not just on the team), or Primal Pokémon \textit{present}
   (even knocked out) at a Raid or gym assault. The Primal bonus is not awarded to the Trainer who brought
   ...
   \end{table}
   Mega forms cannot participate in Max battles; there, $M_\mathrm{M}$ is
    instead based on the number of Pokémon left at that Power Stop (see \autoref{sec:maxbattles}).
   \begin{table}[h!]
     \centering
       \begin{tabular}{lc}
...
The Leagues are sometimes offered in ``Premier'' form, excluding Mythical,
There are variations on the Leagues (\textit{Mega} allows Mega evolved Pokémon;
 \textit{Premier} prohibits Legendary and Mythical Pokémon (\autoref{sec:regions})),
 plus assorted Cups with sundry restrictions (\autoref{table:gblcups}).
\begin{table}
\centering
\footnotesize
\begin{tabular}{ll}
Cup & Restrictions \\
\Midrule
Catch & Must have been caught in ongoing season\\
Color & Fire, Water, Grass, or Electric\\
Electric & Electric\\
Element & Fire, Water, or Grass; prohibits anything evolved\\
Evolution & Must have evolved once, but not be fully evolved\\
Fantasy & Dragon, Fairy, or Steel\\
Fighting & Fighting; prohibits Psychic\\
Flying & Flying\\
Fossil & Rock, Steel, or Water\\
Halloween & Bug, Dark, Fairy, Ghost, or Poison; prohibits Legendary and Mythical\\
Holiday & Electric, Flying, Ghost, Grass, Ice, or Normal\\
Jungle & Bug, Dark, Electric, Flying, Grass, Ground, Normal, or Poison\\
Little & Max CP of 500; must be able to evolve; prohibits anything evolved\\
Love & Red or pink coloring\\
Mountain & Ground, Ice, Rock, or Steel\\
Psychic & Psychic; prohibits Mew\\
Retro & Prohibits Dark, Fairy, and Steel\\
Scroll & Dark, Fighting, or Water\\
Single-type & Prohibits dual typing\\
Spring & Fairy, Grass, or Water\\
Summer & Bug, Electric, Fire, Grass, Normal, or Water\\
Sunshine & Fire, Grass, Ground, or Normal\\
Weather & Fire, Ice, Rock, or Water\\
Willpower & Dark, Fighting, or Psychic\\
\end{tabular}
\caption{Limited GO Battle League Cups\label{table:gblcups}}
\end{table}
...
Link Charges can be used to participate in Mega raids, but only if the
  Trainer is not holding a Raid Pass.
...
A fused (\autoref{sec:fusion}), crowned (\autoref{sec:crowned}), or Mega/Primal (\autoref{sec:mega}) raid
 boss will reward its relevant energy, but Trainers will encounter the base form.
...
\input{out/dynasbug}
\input{out/dynasdark}
\input{out/dynasdragon}
\input{out/dynaselectric}
\input{out/dynasfairy}
\input{out/dynasfighting}
\input{out/dynasfire}
\input{out/dynasflying}
\input{out/dynasghost}
\input{out/dynasgrass}
\input{out/dynasground}
\input{out/dynasice}
\input{out/dynasnormal}
\input{out/dynaspoison}
\input{out/dynaspsychic}
\input{out/dynasrock}
\input{out/dynassteel}
\input{out/dynaswater}
...
Potions and Revives are level-gated, but once unlocked can be semi-randomly acquired
  ...
  from advancing to a new Level (\autoref{table:level40items}, \autoref{table:level80items}),
