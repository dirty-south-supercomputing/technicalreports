...
The \CP{} will be replaced with ``???'' if the \CP{} exceeds that of any owned Pokémon.
...
Six different kinds of lures (\autoref{table:lures}) can be installed into Pokéstops.
...
\end{itemize}
If you throw your ball too directly at some large Pokémon (e.g.\ Tyrunt or Salamence), it can be eaten.
...
Incubators cannot be loaded without an open slot in Pokémon storage, which the egg will fill
 until it hatches.
Trainers start with an Infinite Incubator capable of unlimited use.
...
Timed Incubators are limited by time rather than utilization, disappearing after some date\footnote{If the Timed Incubator is in use when its time expires, the egg within is moved to a single-use incubator, and continues development.}.
...
Enter as many encounters as possible, check the Pokémon summary
  for the Shiny icon (\calign{\includegraphics[width=1em,keepaspectratio]{images/g-shiny.png}}),
