...
Evolution (unlike most form changes) is irreversible.
...
Certain evolutions depend upon a condition (\autoref{table:condevolutions}),
  or walking the Pokémon while your Buddy (\autoref{table:walkevolutions}),
  or a catalyzing item, which will be consumed (\autoref{table:itemevolutions}).
...
\begin{tabular}{lll}
  ...
  Poipole & 200 Poipole + capture 20 \includegraphics[width=1em,height=1em]{images/dragon.png} & Naganadel\\
  ...
  Rockruff & 50 Rockruff + evolve during the day & Lycanroc Midday\\
  Rockruff & 50 Rockruff + evolve at night & Lycanroc Midnight\\
  Rockruff & 50 Rockruff + evolve during dusk & Lycanroc Dusk\\
\end{tabular}
...
Mega evolution and Primal reversion are almost equivalent\footnote{The
 principal difference being that Primals cannot break shields in Super
 Mega raids (\autoref{sec:raids}).}}; where I write Mega,
 read ``Mega [evolution] or Primal [reversion]'' unless otherwise noted.
Species with corresponding Mega or Primal Energy (no species has both) can take on their Mega form for eight hours.
Only one of a Trainer's Pokémon can be thus evolved at a given time---Mega
  Evolving a Pokémon causes any existing Mega form to revert.
...
While in Mega form, Pokémon cannot be used in Max Battles, cannot defend gyms,
 cannot be powered up (though they can be taught second attacks), and cannot be traded.
...
After reaching Mega level 3, level 4 can be unlocked with 5,000 units of the appropriate Mega energy.
...
When actively battling in a raid, Mega Pokémon provide a 30\% boost to attacks of aligned
  ...
  by being in the raiding party (i.e.\ they needn't be active)\footnote{Primal Kyogre
  augments types boosted by rain: Water, Bug, and Electric.
  Primal Groudon benefits types boosted by clear skies, adding Grass to its Ground and Fire.
  Mega Rayquaza affects types boosted by wind: Psychic, Flying, and Dragon.}.
...
Certain Mega Pokémon can attain a fourth level.
Upon reaching Mega Level 3, a one-time cost of 5,000 Mega Energy secures Level 4.
Such Pokémon unlock a ``+ attack'' while Mega Evolved.
% FIXME detail +attacks
...
%Mega Barbaracle, Mega Baxcalibur, Mega Chandelure, Mega Chesnaught, Mega Chimecho, Mega Clefable, Mega Delphox, Mega Dragalge, Mega Drampa, Mega Eelektross, Mega Emboar, Mega Excadrill, Mega Greninja, Mega Feraligatr, Mega Froslass, Mega Hawlucha, Mega Meganium, Mega Pyroar, Mega Scolipede, Mega Scrafty, and Mega Zygarde.
%\input{out/gigantas}
%\vfill
...
Eternatus has its own cost schedule, requiring the same numbers of Max Particles,
 ...
 Level 3 is an absolutely punishing 1,200 Candy plus 320 Candy XL.
...
Max Guard bestows a protective shield (the Guard) on its user.
This Guard will absorb 20, 40, or 60 points of damage, depending on Max Guard's level.
A Pokémon can have up to three Guards at a time, and they persist across substitutions.
If any Guards are active, targeted attacks will only target Pokémon with a Guard.
...
A trade resets any enabled Max Moves to level 1; disabled Moves remain disabled.
...
Crowning consumes 30 of the relevant Candy and 1,000 of the appropriate
  Crown Energy, but once a Pokémon has been crowned, that Pokémon can freely
  change between the base and crowned forms, even if traded.
...
Zygarde can change to a ``50\%'' form for 50 Cells, and a subsequent ``Complete''
...
Each Pokémon of Myth has an ``Origin'' form with different stats and attacks, including
 (in the cases of Palkia and Dialga) access to a signature attack with an Adventure Effect (\autoref{sec:effects}).
...
Origin Forme Palkia can know Spacial Rend, and Origin Forme Dialga Roar of Time, but these
 attacks are not available via Elite Charged TM outside of special events---the Pokémon must know these
 attacks when caught.
...
Rockruff does not have distinguished forms in the Pokédex, but any given Rockruff
 can evolve either into Lycanroc Dusk, or (depending on time of day)
 Lycanroc Midday/Midnight.
Rockruff which can evolve into Lycanroc Dusk are rare.
...
Unown has 28 forms: one for each letter of the Latin alphabet, and two
  (supposedly \texttt{!} and \texttt{?}) resembling deformed sperm.
...
Spinda has twenty visually distinct forms, but only ten have been released.
With bated breath we await the other half.
...
It is not usually possible to trade nor accumulate arbitrary numbers of Mythical
  Pokémon\footnote{Marshadow, Hoopa, Celebi, Mew, Shaymin, Victini, Meloetta, Keldeo,
%  Phione, Manaphy,
  Jirachi, Diancie, Zarude, and Volcanion cannot be traded, and have not appeared outside of Special Research.
  Genesect, Deoxys, and Darkrai cannot be traded, but can be accumulated.}, nor Legendaries Zygarde and
...
%  Hisuian Goodra,
Pseudolegendaries & Dragonite, Tyranitar, Salamence, Metagross, Garchomp, Hydreigon, Goodra, Kommo-O, Dragapult, Baxcalibur\\
\end{tabular}
  \caption{Collections of [pseudo]Legendary Pokémon\label{table:namedmyths}}
\end{table}
...
\section{Battle forms\label{sec:battleforms}}
Several Pokémon have special battle mechanics represented as multiple forms.
These forms are only applicable in 3x3 battles (\autoref{sec:3x3}, and do not
persist. Such Pokémon cannot participate in Nx1 battles (\autoref{sec:nx1}).
These mechanics require explicit support in any good simulator (\autoref{chap:simul}).
...
\subsection{Mimikyu\label{subsec:Mimikyu}}
Mimikyu is shrouded in a rag, and this rag has the ability to absorb one charged attack,
just like a shield. When a charged attack is thrown at Mimikyu, the Trainer can choose
to use a shield if any are available. If the move is not shielded, Mimikyu loses its
disguise (``Busted Form''). This results in a second-level \textit{persistent} debuff (\ref{sec:buffs})
of Mimikyu's defense---this debuff cannot be cleared by switching Pokémon.

\subsection{Cramorant\label{subsec:Cramorant}}
When in its default form, use of either the Surf or Dive charged attack changes Cramorant's form.
If used with at least half its HP remaining, Carmorant enters its Gulping form, with an Arrokuda in its bill.
It otherwise enters its Gorging form, with a Pikachu.
Upon being hit with a charged move (a shielded move does not count), Cramorant will spit its prey at its opponent.
The Gulping form's Arrokuda debuffs the opponent defense by one step,
The Gorging form's Pikachu debuffs the opponent's attack by two steps.
In both cases, damage is dealt equivalent to 15\% of the opponent's \MHP{}, and Cramorant returns to its default form.
This counterattack takes place even if Cramorant is knocked out by the charged attack.
If Cramorant is knocked out by a fast attack (or by a shielded charged move), the counterattack does not occur.
Use of Surf or Dive while already in the alternate form does not change form.
Switching Cramorant out resets its form.

\section{Freak Pokémon a/k/a Freakémonics\label{sec:freaks}}
A few Pokémon have unique behaviors.
