...
Only one of a Trainer's Pokémon can be in its Mega form at any given time---Mega
  evolving a Pokémon will cause any existing Mega form to revert.
...
When actively battling in a raid, Mega Pokémon provide a 30\% boost to attacks of aligned
  ...
  augments types boosted by rainy weather: Water, Bug, and Electric.
  Primal Groudon benefits types boosted by clear weather, adding Grass to its Ground and Fire.
  Mega Rayquaza affects types boosted by windy weather: Psychic, Flying, and Dragon.}.
...
Crowning consumes 30 of the appropriate Candy and 1,000 of the appropriate
...
If used with at least half its HP remaining, Carmorant enters its Gulping form.
It otherwise enters its Gorging form.
Upon being hit with a charged move (the move must not be shielded),
 the Gulping form debuffs the opponent defense by one step,
 while the Gorging form debuffs the opponent's attack by two steps.
Either way, damage is dealt equivalent to 15\% of the opponent's \MHP{}.
Cramorant then returns to its default form.
This counterattack takes place even if Cramorant is knocked out by the charged attack
 (unless it is knocked out while shielding).
