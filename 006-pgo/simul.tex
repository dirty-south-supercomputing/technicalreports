...
\item Fast attack, and shield (if we have one): \texttt{MOVE\_FAST\_SHIELD}
\end{itemize}
...
\item Charged attack one, and shield (if we have one): \texttt{MOVE\_CHARGED1\_SHIELD}
\item Charged attack two, no shield: \texttt{MOVE\_CHARGED2}
\item Charged attack two, and shield (if we have one): \texttt{MOVE\_CHARGED2\_SHIELD}
\end{itemize}
...
\begin{tcblisting}{title=moves.h,listing options={language=C++}}
\input{s/moves.h}
\end{tcblisting}
