...
Finally, it never diverges, instead advancing always towards a conclusion\footnote{There
  is an exception to convergence: both Trainers choosing to do nothing or use
  zero power fast attacks despite full energy. Continuously choosing either
  is pathological, so we simply dismiss all choice pairs that change no state,
  neatly sidestepping this issue.}.
...
So, knowing $T_{max}$, we can determine those cases where no charged attack
  can be used: whenever $T_{max} * E_{fast} < E_{charged}$ for both Pokémon.
