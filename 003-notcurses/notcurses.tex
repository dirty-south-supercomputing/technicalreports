...
\rhead{
  ...
  \textcolor{dsscawpurp}{DSSCAW Technical Report \#003}
}
...
\newglossaryentry{cooked}
{
  name={Cooked mode},
description={ The default terminal mode under SUSv4 (see \textbf{raw mode}).
...
description={ A synonym for \textbf{raw mode}.}
}
...
{
  name={Raw mode},
description={ A collection of behaviors differing from \textbf{cooked mode}. Noticeably,
...
{
  name={termcap},
description={ A deprecated terminal capability database, superseded by terminfo.}
}
...
\begin{denseitemize}
\item \texttt{wchar\_t}: A data type in C and C++. FIXME
