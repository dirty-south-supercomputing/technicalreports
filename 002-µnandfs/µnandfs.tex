...
\date{}
\begin{abstract}
\noindent I was tasked with designing and implementing a persistent associative array
...
Block erasure operations can fail, setting the \texttt{FAIL-E} bit; likewise page
programming and \texttt{FAIL-P}. Both of these bits are reset when the next operation
is started. Page read operations that fail their ECC check result in one or
more \texttt{ECC-CE} error bits being set. These error bits remain high until the chip is
reset. Most operations set the \texttt{BUSY} bit, and only when this bit goes low is it
...
{\texttt{blob\_t}}\footnote{We encode all handle state in the \texttt{blob\_t},
  held by the client. ``File descriptors'' imply space
...
is guaranteed perfectly uniform wear leveling, and should require minimal
state\footnote{I knew that any frequently-changing state couldn't be persisted without either
...
\item \textit{More than two zones have a nonce of all 1s, these zones are
    ...
    decreasing procession.} The µnandfs is either freshly initialized, or was
    interrupted in its first epoch. The current zone is the first zone with an
    all-1s nonce.
...
\begin{center}
\makebox[\textwidth][c]{\includegraphics[width=1\linewidth]{ExtendBlob.png}}
